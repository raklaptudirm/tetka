...
impl Info {
    ...
    pub fn from_squares(
        ...
        info.rooks[Dimension::WhiteH] = Square::new(w_rook_h, Rank::First);
        info.rooks[Dimension::WhiteA] = Square::new(w_rook_a, Rank::First);
        info.rooks[Dimension::BlackH] = Square::new(b_rook_h, Rank::Eighth);
        info.rooks[Dimension::BlackA] = Square::new(b_rook_a, Rank::Eighth);

        // Initialize the castling path table.
        info.blocker_mask[Dimension::WhiteH] = blocker_mask(
            w_king,
            info.rooks[Dimension::WhiteH],
            ends::WHITE_KING_H,
            ends::WHITE_ROOK_H,
        );
        info.blocker_mask[Dimension::WhiteA] = blocker_mask(
            w_king,
            info.rooks[Dimension::WhiteA],
            ends::WHITE_KING_A,
            ends::WHITE_ROOK_A,
        );
        info.blocker_mask[Dimension::BlackH] = blocker_mask(
            b_king,
            info.rooks[Dimension::BlackH],
            ends::BLACK_KING_H,
            ends::BLACK_ROOK_H,
        );
        info.blocker_mask[Dimension::BlackA] = blocker_mask(
            b_king,
            info.rooks[Dimension::BlackA],
            ends::BLACK_KING_A,
            ends::BLACK_ROOK_A,
        );

        info.attacks_mask[Dimension::WhiteH] =
            BitBoard::between2(w_king, ends::WHITE_KING_H);
        info.attacks_mask[Dimension::WhiteA] =
            BitBoard::between2(w_king, ends::WHITE_KING_A);
        info.attacks_mask[Dimension::BlackH] =
            BitBoard::between2(b_king, ends::BLACK_KING_H);
        info.attacks_mask[Dimension::BlackA] =
            BitBoard::between2(b_king, ends::BLACK_KING_A);
        ...
        fn blocker_mask(
            ...
            (BitBoard::between2(king, king_end)
                | BitBoard::between2(rook, rook_end))
                - (BitBoard::from(king) | BitBoard::from(rook))
        }

        // Initialize the rights update for the king's squares.
        info.rights_masks[w_king] =
            Rights::new() | Dimension::WhiteH | Dimension::WhiteA;
        info.rights_masks[b_king] =
            Rights::new() | Dimension::BlackH | Dimension::BlackA;

        // Initialize the rights update for the rook's squares.
        info.rights_masks[info.rooks[Dimension::WhiteH]] =
            Rights::new() | Dimension::WhiteH;
        info.rights_masks[info.rooks[Dimension::WhiteA]] =
            Rights::new() | Dimension::WhiteA;
        info.rights_masks[info.rooks[Dimension::BlackH]] =
            Rights::new() | Dimension::BlackH;
        info.rights_masks[info.rooks[Dimension::BlackA]] =
            Rights::new() | Dimension::BlackA;
