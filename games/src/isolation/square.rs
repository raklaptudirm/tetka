...
representable_type!(
    /// File represents a file on the Isolation Board. Each vertical column of
    /// Squares on an Isolation Board is known as a File.
    enum File: u8 { A "a", B "b", C "c", D "d", E "e", F "f", G "g", H "h", }
);

representable_type!(
    /// Rank represents a rank on the Isolation Board. Each horizontal row of Squares
    /// on an Isolation Board is known as a Rank.
    enum Rank: u8 {
